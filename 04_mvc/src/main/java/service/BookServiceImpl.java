...
public class BookServiceImpl implements BookService {
  ...
  @Override
  public ActionForward getBooks(HttpServletRequest request, HttpServletResponse response) { // 4권 모두 반환
    // JSP 로 전달할 데이터는 request 에 저장한다.
    request.setAttribute("books", books);
    // 어디로 : /book/list.jsp
    // 어떻게 : forward
    return new ActionForward("/book/list.jsp", false); // false 는 Forward 를 의미함
  }

  @Override
  public ActionForward getBookByNo(HttpServletRequest request, HttpServletResponse response) { // 해당 번호의 책만 반환
